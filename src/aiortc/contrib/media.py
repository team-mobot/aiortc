...
class PassthroughMediaRecorder:
    """
    A media sink that can handle MediaStreamTracks that return av.packet.Packets
    and writes audio and/or video to an output container.
    ...
    .. code-block:: python
        ...
        # Write to a video file.
        player = PassthroughMediaRecorder('/path/to/file.mp4')
    ...
    def addTrack(self, track):
        ...
        if track.kind == "audio":
            ...
            # TODO/FIXME: how to know what kind of pre-encoded packets we might
            # get? What container format will be appropriate?
            #
            # For now, we know we're sending h264 encoded video, and we know it is
            # in yuv420p. Is there some way to get pix_fmt from the first packet,
            # like we get dimensions below?
            stream = self.__container.add_stream("libx264", rate=5)
            stream.pix_fmt = "yuv420p"
        self.__tracks[track] = MediaRecorderContext(stream)
    ...
    async def __run_track(self, track: MediaStreamTrack, context: MediaRecorderContext):
        def handle_frame(frame: Frame, context: MediaRecorderContext):
            if not context.started:
                if isinstance(frame, VideoFrame):
                    context.stream.width = frame.width
                    context.stream.height = frame.height
        ...
        def handle_packet(packet: Packet, context: MediaRecorderContext):
            if not context.started:
                # decode just this first packet to get dimensions
                codec = av.CodecContext.create("h264", "r")
                try:
                    first_frame = codec.decode(packet)[0]
                    (w, h) = (first_frame.width, first_frame.height)
                except Exception as e:
                    logging.warning(f"PassthroughMediaRecorder unable to decode first packet to find dimensions: {packet}, {e}")
                    (w, h) = (640, 480)
                context.stream.width = w
                context.stream.height = h
                context.started = True
            logging.debug(f"PassthroughMediaRecorder muxing packet {packet}")
            packet.stream = context.stream
            self.__container.mux(packet)
        ...
        while True:
            ...
            if isinstance(frame_or_packet, av.packet.Packet):
                handle_packet(frame_or_packet, context)
            elif isinstance(frame_or_packet, av.frame.Frame):
                handle_frame(frame_or_packet, context)
            else:
                logging.error("PassthroughMediaRecorder frame_or_packet is not an av Frame OR Packet: {frame_or_packet}")
